...
<head>
    <meta charset="UTF-8">
    <title>${requestScope.official.jobName} 绩效考核表</title>
...
<!-- end of modal -->
<div class="noprint absoPosition">
    <button id="dy_print" class="btn btn-icon waves-effect waves-light btn-primary m-b-5"> <i class="glyphicon glyphicon-print"></i> 打印(打印时请仔细查看预览图)</button>
</div>
...
<div class="container">
                ...
                <div class="col-md-10 col-md-offset-1">
                    ...
                    <table border="2px" bordercolor="0x4C96FF" align = "center">
                        ...
                        <tr>
                            ...
                            <td tdName="ex" dyContent="" dyEx="${requestScope.official.exGrade}" colspan="7" class="relative height80"><b class="absolute ques">特别加/减分理由：</b><b>${requestScope.official.exGrade}分。 </b> ${requestScope.official.exReason}</td>
                        </tr>
                        <tr>
                            <td colspan="2"><b>部门负责人：</b></td>
                            <td  style="width: 25%">${requestScope.official.boss2Name}</td>
                            <td><b>人力行政部：</b></td>
                            <td colspan="3">${requestScope.official.hrName}</td>
                        </tr>
                        <tr>
                            ...
                            <td colspan="2" id="finalGrade2">${requestScope.official.finalGrade}</td>
                        </tr>
                        <tr>
    ...
    <script type="text/javascript">
    	...
    	$(function(){
    		...
    		//自评分小记
			function totalSelfGrade(){
				var total = 0;
				$("td[tdName=selfGrade]").each(function(){
					if($(this).text() == "")
						return;

					total += parseInt($(this).text());
				});
				$("#dy_totalGrade").text(total);
			}
			function totalBoss1Grade(){
				var total = 0;
				$("td[tdName=boss1Grade]").each(function(){
					if($(this).text() == "")
						return;

					total += parseInt($(this).text());
				});
				$("#dy_totalBoss1Grade").text(total);
			}
            //统计 自评分20% + 直属上司分80%
            function totalFinalGrade1(){
                var selfGrade = $("#dy_totalGrade").text();
                var bossGrade = $("#dy_totalBoss1Grade").text();

                var finalGrade = new Number(selfGrade)*0.2 + new Number(bossGrade)*0.8;

                $("#finalGrade1").text(finalGrade.toFixed(2));
            }

            //打印按钮
            $("#dy_print").click(function(){
                window.print();
            });
    	});
    </script>
