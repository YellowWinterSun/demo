                                            ...
                                            <section>
                                                <div class="form-group clearfix">
                                                    ...
                                                    <div class="col-lg-6">
	                                                    ...
	                                                    <!-- bootstrap-select2 提供图片option功能，暂时替代上方的select成为新的方案 -->
                                                        <select id="dy_select2demo" class="js-example-templating js-states form-control" >
                                                        	<c:forEach items="${requestScope.listRoom}" var="item">
                                                                <c:choose>
                                                                    <c:when test="${requestScope.roomId != null && requestScope.roomId == item.id}">
                                                                        <option title='<c:out value="${item.path}"/>' selected value='<c:out value="${item.id}"/>'><c:out value="${item.name}"/></option>
                                                                    </c:when>
                                                                    <c:otherwise>
                                                                        <option title='<c:out value="${item.path}"/>' value='<c:out value="${item.id}"/>'><c:out value="${item.name}"/></option>
                                                                    </c:otherwise>
                                                                </c:choose>
                                                            </c:forEach>
                                                        </select>
